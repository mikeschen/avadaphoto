		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					...
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'textbook' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					...
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'textbook' ) . '</a>' : ''
				);
			}
...
function textbook_enqueue_scripts() {
...
add_action( 'admin_enqueue_scripts', 'textbook_enqueue_scripts' );
 ...
 */
function textbook_dismiss_admin_notice() {
...
add_action( 'wp_ajax_textbook_jetpack_admin_notice', 'textbook_dismiss_admin_notice' );
